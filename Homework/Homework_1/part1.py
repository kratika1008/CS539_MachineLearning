...
class Tree(object):
    ...
    def entropy(Y):
        ...
        lstVal = Counter(Y)
        allVals = list(lstVal.keys())
        total = len(Y)
        e=0
        for eachVal in allVals:
            p =  lstVal[eachVal]/total
            e += -p * math.log2(p)
        #########################################
        return e
    #--------------------------
    ...
    def conditional_entropy(Y,X):
        ...
        #########################################
        ## INSERT YOUR CODE HERE
        allUniqueX = np.unique(X)
        total = len(Y)
        ce=0
        for eachX in allUniqueX:
            relY = Y[X==eachX]
            p =  len(relY)/total
            ce += p * Tree.entropy(relY)
        #########################################
        return ce
    ...
    def split(X,Y,i):
        ...
        C=dict()
        for eachAtt in uniqueAtt:
            relY = Y[att==eachAtt]
            relX = X[:,att==eachAtt]
            C[eachAtt] = Node(relX,relY)
    ...
    def stop2(X):
        ...
        for i in range(len(X)):
            state = Tree.stop1(X[i])
            states.append(state)
        c = list(Counter(states).keys())
        if np.logical_and(len(c)==1, c[0]):
            s=True
        #########################################
        return s
    ...
    def build_tree(t):
        ...
        if np.logical_or(Tree.stop1(t.Y), Tree.stop2(t.X)):
            t.isleaf = True
        else:
            t.isleaf = False
    ...
    def train(X, Y):
        ...
        if np.logical_or(Tree.stop1(Y), Tree.stop2(X)):
            t.isleaf = True
        else:
            t.isleaf = False
    ...
    def load_dataset(filename = 'data1.csv'):
        ...
        #########################################
        ## INSERT YOUR CODE HERE
        A = np.loadtxt(filename, delimiter=',', dtype='object',skiprows=1)
        X = A[:,1::].T
        Y = A[:,0]
